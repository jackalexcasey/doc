...
const char optstring[] = "c:d:ht:vx";
...
typedef struct test_info {
	...
	int64_t		sched;		/* worst sched latency	*/
	int64_t		sum;		/* worst sched latency	*/
...
void show_sched(struct test_info *test)
{
	if(!xverbose)
	INFO("new %s-sched maximum: %9"PRId64, test->name, test->sched);
}
...
{							\
	...
	spin_unlock(&test->lock);			\
							\
	delta = t1 - t0;				\
	if(t0 && delta >0 && delta >test->sched){ \
		test->sched=delta; \
		show_sched(test); \
	}\
	if(t0 && delta >0 && delta>threshold){ \
		test->sum += delta; \
		if(xverbose) fprintf(stdout,"%8Ld		%8Ld\n",delta,test->sum); \
	}\
